...
class Charging:
    ...
    def charge_ending(self, call_type: CallType) -> VehicleState:
        ...
        if call_type == CallType.Incoming:
            ...
            charging_efficiency = (set_state(Hash.CS_ChargingEfficiency, (wh_added / wh_used) if wh_used > 0 else 0.0)) * 100.0
            ...
            _LOGGER.info(f"    odometer: {odometer_km(odometer):.1f} km ({odometer_miles(odometer):.1f} mi)")
            _LOGGER.info(f"    starting HvB temperature: {hvb_starting_temp}°C, ending HvB temperature: {hvb_ending_temp}°C")
            _LOGGER.info(f"    starting HVB SoC: {hvb_starting_soc:.1f}%, ending SoC: {hvb_ending_soc:.1f}%")
            ...
            _LOGGER.info(f"    starting LVB SoC: {lvb_starting_soc:.1f}%, ending SoC: {lvb_ending_soc:.1f}%")
            _LOGGER.info(f"    {wh_added} Wh were added, requiring {wh_used} Wh from the charger")
            _LOGGER.info(f"    overall efficiency: {charging_efficiency:.1f}%")
